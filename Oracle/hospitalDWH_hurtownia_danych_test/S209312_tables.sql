...
(
    ...
    lekarz VARCHAR2(8),
    FOREIGN KEY (lekarz) REFERENCES lekarze(nrkadr)
);
...
(
    ...
    FOREIGN KEY (bad_wykon) REFERENCES S209312.lekarze(nrkadr)
);
...
(
    ...
    FOREIGN KEY (pesel) REFERENCES S209312.pacjenci(pesel),
    FOREIGN KEY (bad_diag) REFERENCES S209312.badania(badanieid)
);
...
COMMENT ON COLUMN S209312.wypisy_w_pelni_zdrowi.diagid IS 'ID zdiagnozowanego schorzenia, na ktore pacjent byl leczony';
...
COMMENT ON COLUMN S209312.wypisy_dalsze_leczenie.diagid IS 'ID zdiagnozowanego schorzenia, na ktore pacjent byl leczony';
...
COMMENT ON COLUMN S209312.zgony.data_danych IS 'Data sprawozdawcza';
COMMENT ON COLUMN S209312.zgony.pesel IS 'PESEL pacjenta';
COMMENT ON COLUMN S209312.zgony.data_rejestracji IS 'Data rejestracji pacjenta';
COMMENT ON COLUMN S209312.zgony.data_smierci IS 'Data smierci pacjenta';
COMMENT ON COLUMN S209312.zgony.przyczyna IS 'Przyczyna smierci';
COMMENT ON COLUMN S209312.zgony.diagid IS 'ID zdiagnozowanego schorzenia, na ktore pacjent byl leczony';
COMMENT ON COLUMN S209312.zgony.nr_hist IS 'Nr historii leczenia pacjenta';
COMMIT;
...
create public database link hosp_prod connect to S209312 identified by KacperADMIN
using
'(DESCRIPTION =
    (ADDRESS = (PROTOCOL = TCP)(HOST = localhost)(PORT = 1521))
    (CONNECT_DATA =
      (SERVER = DEDICATED)
      (SERVICE_NAME = hospitalDWH_prod)
    )
  )';