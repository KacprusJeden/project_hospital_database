create or replace function  Patients.get_patient_age(v_pesel varchar2)
RETURN number
IS
    -- sprawdzam, czy pesel nalezy do pacjenta niezyjacego na zasadzie: czy jest przypisana data smierci dla rekordu o podan
    cursor v_cur is select deathdate from patients.death where pesel = v_pesel; 
    var_date_end death.deathdate%type;
    v_datafound boolean;
BEGIN
  OPEN v_cur;
  loop
